...
import os
import re
from esper import EsperAutoAppUpdater
...
def lambda_handler(event, context):
    ...
    if device_id is None:
        return create_return_json(400, {"status" : "Request failed", "reason" : "Payload is missing Device ID, or the device ID is malformed"})
    print(f"POST: device_id - {device_id}")

    pkg = get_pkg_from_body(body)
    if pkg is None:
        return create_return_json(400, {"status" : "Request failed", "reason" : "Payload is missing pakage name, or the package name is malformed"})
    print(f"POST: pkg - {pkg}")

    build_number = get_build_number_from_body(body)
    if pkg is None:
        return create_return_json(400, {"status" : "Request failed", "reason" : "Payload is missing build number, or the build number is malformed"})
    ...
    if "Suceeded" == status:
        return create_return_json(200, {"status" : "Request succeded", "msg" : msg })
    else:
